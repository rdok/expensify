...
function filter(expenses, filters) {
  return expenses.filter((expense) => {
    ...
    return ['description', 'note'].find((field) => expense[field].toLowerCase().includes(text)) !== undefined;
  });
}

function sort(expenses) {
  return expenses.sort((expense, nextExpense) => (expense.createdAt.isBefore(nextExpense.createdAt) ? 1 : -1));
}
